...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
  <head>
    ...
    <style>
      .gradient {
        background: linear-gradient(90deg, #d53369 0%, #51a6da  100%);
      }
    </style>
    {{-- Scripts  --}}
  ...
  <body class="leading-normal tracking-normal text-white gradient" style="font-family: 'Source Sans Pro', sans-serif;">
      ...
      <div class="w-full container mx-auto flex flex-wrap items-center justify-between mt-0 py-2">
        <div class="pl-4 flex items-center">
          <a class="toggleColour text-white no-underline hover:no-underline font-bold text-2xl lg:text-4xl" href="#">
            <x-css.icon type="hub" style="font-size:26px" />
            PRESTA-CONNECT
          </a>
        </div>
            ...
            <li class="mr-3">
              <a class="inline-block text-black no-underline hover:text-gray-800 hover:text-underline py-2 px-4" href="#">link</a>
                ...
                @if (Route::has('login'))
                    @auth
                      <li class="mr-3">
                          <a id="navAction" href="{{ url('/dashboard') }}" class="inline-block no-underline hover:text-gray-800 hover:text-underline py-2 px-4 bg-white text-gray-800">Dashboard</a> 
                      </li>                       
                          <!-- Authentication -->
                      <li class="mr-3">
                        <form method="POST" action="{{ route('logout') }}">
                          @csrf
                          <button type="submit" class="inline-block text-black no-underline hover:text-gray-800 hover:text-underline py-2 px-4">{{ __('Logout') }}</a>
                        </form>
                      </li>
                    @else
    ...
    <div class="pt-24">
      <div class="container px-3 mx-auto flex flex-wrap flex-col md:flex-row items-center">
        ...
        <div class="w-full md:w-3/5 py-6 text-center">
          <img class="w-full md:w-4/5 z-50" style="max-width: 613px;float: right;" src="/img/hero.png" />
        </div>
      </div>
    </div>
    ...
    <section class="container mx-auto text-center py-6 mb-12">
      ...
      <h3 class="w-full my-2 text-5xl font-bold leading-tight text-center text-white">
        {{ __('Contact for more information') }}
      </h3>
      <div class="w-full mb-4">
        <div class="h-1 mx-auto bg-white w-1/6 opacity-25 my-0 py-0 rounded-t"></div>
      </div>
      
      @include('front.forms.contactForm', [ 'roles' => '' ])
